...
class SwingBacktester:
    def __init__(self, csv_path, swing_window=6, sl_multiplier=2):
    ...
    def load_data(self):
        self.data = pd.read_csv(self.csv_path, index_col='timestamp', parse_dates=True)
        self.data = self.data[['o', 'h', 'l', 'c']]
    ...
    def detect_swings(self):
        detect_swing(self.data, 'high', self.swing_window)
        detect_swing(self.data, 'low', self.swing_window)
    ...
    def run_backtest(self):
        ...
        for i in range(1, len(data)):
            ...
            if not in_position and signal != 0:
                ...
                entry_time = time_now
                SL_price = entry_price - (entry_candle_height * self.sl_multiplier) if position == 1 else entry_price + (entry_candle_height * self.sl_multiplier)
                in_position = True
            elif in_position:
